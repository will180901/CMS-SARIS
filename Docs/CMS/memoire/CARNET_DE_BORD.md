# Carnet de bord — mémoire CMS SARIS

> **Dernière mise à jour** : 24 août 2026
> **Document** : `Memoire_CMS_SARIS.docx` — **98 pages**, dont **84 pages de corps**
> **Reste à faire** : tracer 20 diagrammes, produire 3 captures, les coller, régénérer les listes.

---

## 1. Identité du mémoire — figée

| Élément | Valeur |
|---|---|
| **Thème** | Conception et réalisation d'un système de gestion des consultations et des dossiers médicaux : cas du Centre Médico-Sanitaire (CMS) **de la SARIS** |
| Établissement | CFI-CIRAS · Licence professionnelle en informatique · option Génie Logiciel Applicatif |
| **Auteurs** | NZILA Oscarvie Verdi · BOUWAYI MIKOUYA Déo Cherel |
| **Promoteur** | M. Roch Corneille NGOUBOU |
| Promotion | 2025 *(mention conservée volontairement)* |
| Année académique | 2025-2026 |
| Stage | 15 janvier au 14 avril 2026, effectué par **Nzila Oscarvie Verdi seul** |

**La page de garde ne se modifie pas.** Une seule exception à ce jour : le 19 août 2026, les mots « DE LA SARIS » ont été rétablis à la fin du thème, qui s'arrêtait à « (CMS) ».

---

## 2. Où en est le document

| Fait | |
|---|---|
| Les huit chapitres, l'introduction et la conclusion, rédigés puis refondus | ✅ |
| Mise en page : styles, marges, en-tête, pied de page | ✅ |
| Sommaire, liste des figures, liste des tableaux, table des matières | ✅ champs automatiques |
| Pagination en trois parties : **i à viii**, **1 à 84**, **A à E** | ✅ |
| Chaque chapitre démarre en haut d'une page neuve | ✅ règle portée par le style |
| Chaque figure a son emplacement réservé | ✅ 23 emplacements, dont 9 en demi-page |
| Bibliographie et webographie, une page chacune | ✅ |
| Dossier documentaire aligné sur le document | ✅ |

### Ce qui reste

| # | Quoi | Qui |
|---|---|---|
| 1 | Tracer les **20 diagrammes** — fiches prêtes dans `05_fiches_de_dessin/` | Toi |
| 2 | Produire les **3 captures** — protocole dans `06_interfaces/` | Toi |
| 3 | Coller les 23 images au-dessus de leur légende, puis **Ctrl+A** et **F9** | Toi |
| 4 | Lire les **4 références** de la bibliographie | Toi |
| 5 | Obtenir de Verdi : infrastructure réseau, effectifs chiffrés, guide d'entretien | Toi |
| 6 | Demander au promoteur si la limite de 70-85 pages vise le corps ou le document entier | Toi |
| 7 | Relecture finale et contrôle de pagination | Moi, après tes images |

---

## 3. Le point de vigilance

Le corps fait **84 pages**. Le plan de l'école exige entre 75 et 90 ; ton promoteur a dit 70 à 85.

**La marge est d'une seule page** avant le plafond du promoteur. Ne retire rien sans compenser. Si le volume doit augmenter, les trois réserves légitimes sont les sections encore en attente de source : l'infrastructure réseau au chapitre 2, les effectifs au chapitre 1, le guide d'entretien au chapitre 5.

---

## 4. Règles de travail

1. **Une chose à la fois**, validée avant la suivante.
2. **Le document Word fait foi sur le texte. Les inventaires font foi sur les chiffres.** Une divergence entre les deux est un écart à instruire, jamais à masquer.
3. **Tu colles toi-même** les figures, les captures et les blocs de code.
4. **Rien ne s'invente.** Une information indisponible reste marquée en attente de source.
5. Après toute suppression, **vérifier les renvois** : aucune référence ne doit pointer dans le vide.
6. **Sommaire, listes et sigles se régénèrent en dernier**, quand plus rien ne bouge.

---

## 5. Mise en page

A4 · marges 1,5 cm, reliure gauche 1,5 · Times New Roman 12 · justifié · interligne 1,25 · espacement après 6 pt.

Styles : **Titre 1** (16 gras centré, saut de page avant) · **Titre 2** (14 gras) · **Titre 3** (12 gras) · **Légende** (11 italique, *figures*) · **Légende tableau** (11 italique, *tableaux*).

En-tête : « Mémoire de Fin de Cycle — GLA — CFI-CIRAS ». Pied de page : numéro centré, année académique à droite.

**Quatre sections Word** portent la pagination : page de garde sans numéro, liminaires en romains, corps en arabes redémarrant à 1, bibliographie et suite en lettres.

---

## 6. Les 23 figures à produire

**Vingt diagrammes à tracer.** Le mémoire est passé de 15 à 23 illustrations le 24 août : les cas d'utilisation et les classes sont désormais découpés **par package**, comme dans le mémoire de référence de NGATSE et KUBEMBULA.

| Figure | Fiche |
|---|---|
| 1.1 Organigramme du Service Médico-Social | `ORG-01` |
| 4.1 Cycle de développement selon 2TUP | `ORG-03` |
| 5.1 Activité du processus de consultation antérieur | `UML-ACT-01` |
| 6.1 Diagramme de contexte statique | `UML-CTX-01` |
| 6.2 à 6.6 Cas d'utilisation — **cinq packages** | `UML-UC-01` |
| 6.7 Séquence système : émettre un bon de pharmacie | `UML-SEQS-02` |
| 6.8 Séquence système : synchroniser un poste local | `UML-SEQS-03` |
| 7.1 à 7.4 Classes — **quatre packages** | `UML-CLS-01` |
| 7.5 Diagramme de classes global | `UML-CLS-01` |
| 7.6 Diagramme de composants | `UML-CMP-01` |
| 7.7 Diagramme de déploiement | `UML-DEP-01` |
| 8.1 Schéma relationnel du noyau métier | `SCH-REL-01` |
| 8.2 Modèle physique de données | `SCH-MPD-01` |

**Trois captures**, protocole dans `06_interfaces/protocole_captures.md` :

- 8.3 Consultation en cours *(rôle Médecin Chef)*
- 8.4 Émission d'un bon de pharmacie et contrôle d'éligibilité *(rôle Infirmier)*
- 8.5 Tableau de bord et journal d'audit *(rôle Administrateur système)*

⚠️ Aucune donnée réelle de patient sur les captures.

**Les neuf diagrammes de package occupent une demi-page chacun, deux par page.** Prévois des images d'environ 16 cm sur 11. Les autres gardent une page entière.

## 7. Régénérer les listes dans Word

| Élément | Chemin |
|---|---|
| Sommaire | Références → Table des matières personnalisée → **1 niveau** |
| Table des matières | Idem → **3 niveaux** |
| Liste des figures | Références → Table des illustrations → Options → **Style : Légende** |
| Liste des tableaux | Idem → **Style : Légende tableau** |

⚠️ Toujours passer par **Options → Style**, jamais par « Étiquette ».
⚠️ Si Word demande « Voulez-vous remplacer cette table des matières ? », répondre **Non** — sinon il écrase le sommaire.

Pour tout mettre à jour d'un coup : **Ctrl + A**, puis **F9**, puis « Mettre à jour toute la table » quatre fois.

---

## 8. Vocabulaire verrouillé

Le mémoire emploie **25 sigles**, tous définis à leur première apparition et tous listés en page viii. Les dix sigles techniques — API, REST, HTTP, JSON, JWT, SQL, ORM, SSE, IPC, PWA — sont justifiés par un relevé dans le code, consigné dans `01_preliminaires/sigles_et_abreviations.md`. Deux règles : **un sigle n'entre dans la liste que s'il est employé dans le texte**, et **aucun sigle qui ne soit pas dans le code** — SGBD et IHM sont donc exclus.

| Interdit | À employer |
|---|---|
| SGCDM | CMS SARIS |
| interface de programmation | **API** |
| jeton signé | **jeton JWT** |
| flux d'événements | **flux SSE** |
| Brazzaville, Pointe-Noire *pour les sites du centre* | Moutela, Nkayi |
| Centre Médico-**Social** | Centre Médico-**Sanitaire** |
| « utilisé par le personnel du centre » | **conçu et développé** — l'application *est* déployée sur Render, mais aucun usage clinique n'est établi |

Ne jamais confondre **ordonnance** et **bon de prise en charge**, ni **verrou de dossier** et **rideau de confidentialité**. Aucune notion de priorité au triage. `MEDECIN` n'est pas un rôle du système.

---

## 9. Chiffres qui font foi

268 routes · 88 entités · 97 associations · 128 permissions · 15 écrans · 41 migrations · 52 entités synchronisées · **29 classes** au diagramme, donc 59 écartées · environ 93 500 lignes de code · 145 cas de test écrits, dont **103 exécutés et 103 réussis** le 10 août 2026.

Ces valeurs viennent d'un comptage direct dans le code et remplacent toute estimation antérieure.

---

## 10. Organisation du dossier

| Dossier | Contenu |
|---|---|
| `00_pilotage_et_preuves/` | Tableau de bord, registres, matrices, les 8 inventaires |
| `01_preliminaires/` à `09_conclusion_et_references/` | Le texte du mémoire, aligné sur le Word |
| `05_fiches_de_dessin/` | Index des 23 figures et les 12 fiches de diagrammes |
| `06_interfaces/` | Protocole des 3 captures |
| `11_revue_finale/` | Checklist, rapports de cohérence, **diagnostic d'alignement** |
| `99_archive/` | Ce qui a été retiré du mémoire, avec le motif de chaque retrait |
