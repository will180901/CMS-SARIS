...
- **Vue clinique** (pour les soignants : médecin-chef, médecin, infirmier) : KPI du jour, file d'attente,
...
- **Tableaux de bord par rôle supprimés** : les vues `AGENT_RH` et `ADMIN_MEDICAL` ont été **retirées**
  (réduction à 4 rôles, [[registre_decisions]] D-003) ; il ne reste que **ClinicalView** et
  **AdminSystemView**.
...
Rôles du système (4, cf. [[glossaire]] « Rôle », [[registre_decisions]] D-003, `PM-46`). L'accès au tableau
...
| **MEDECIN_CHEF** | Vue **clinique** | `dashboard.read` (overview, motifs, urgences, tendance, affluence) ; `consultation.read` (statistiques d'activité) ; widget délégations si `delegation.read`. |
| **MEDECIN** | Vue **clinique** | Idem MEDECIN_CHEF (mêmes permissions cliniques). *Réserve catalogue : voir [[registre_decisions]] D-003.* |
...
- **EF-15-01** — Le système expose `GET /dashboard/overview` qui renvoie les KPI cliniques **du jour** du
  site de l'appelant : nombre de visites du jour (attente / en cours / clôturées / annulées + total),
  visites de la veille, **tendance % vs veille**, **temps d'attente moyen** (minutes, sur visites clôturées
  du jour), consultations actives, consultations clôturées du jour, ordonnances validées du jour, bons
  d'examen en attente, évacuations en cours. (`getOverview`)
...
- **EF-15-08** — Le système expose `GET /dashboard/statistiques?from&to` : agrégation des consultations sur
  une **période** (défaut : **30 derniers jours**) par **type de consultation**, par **pathologie**
  (diagnostic `PRINCIPAL`, top 15) et par **catégorie de patient**, plus la synthèse **repos** (nombre de
  consultations avec repos + total des jours). (`getStatistiques`)
...
- **EF-15-10** — Le système expose `GET /dashboard/admin-systeme` (réservé `utilisateur.read`) : comptes
  (actifs / bloqués / désactivés / total), nombre de rôles, **échecs de connexion sur 24 h**, **sessions
  actives**, **volume d'actions d'audit sur 7 jours**, et la **tendance d'authentification** (succès /
  échecs par jour sur 7 jours). (`getAdminSystemStats`)
...
- **EF-15-12** — L'interface affiche un **persona unique** par utilisateur : `ADMIN_SYSTEME` → vue admin
  système ; `MEDECIN_CHEF` / `MEDECIN` / `INFIRMIER` → vue clinique ; repli par permissions pour rôles
...
- **Acteur** : MEDECIN_CHEF, MEDECIN, INFIRMIER (vue clinique).
...
- **Scénarios d'erreur** : aucune consultation sur la période → message « Aucune consultation clôturée sur
  la période » et boutons d'export désactivés (EF-15-13).
...
- **RM-15-04** — **Période par défaut des statistiques** : **30 derniers jours** si `from`/`to` absents ;
  `from` borné à 00:00:00, `to` à 23:59:59.999. *(Valeur 30 j : définition locale du défaut backend ;
