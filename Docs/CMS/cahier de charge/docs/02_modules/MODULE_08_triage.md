...
Rôles du système (3, cf. [[glossaire#Rôle]], PM-46) ; le contrôle d'accès s'appuie sur le garde
`@RequirePermissions` (permissions `visite.*`, [[MODULE_02_acces_habilitations]]). « MEDECIN » n'est
**pas** un rôle : c'est une **profession** du personnel médical mappée au rôle `MEDECIN_CHEF` (tous les
médecins reçoivent ce rôle).
...
| **MEDECIN_CHEF** | Accueil/triage + **supervision** : voit tout l'historique du site ; dispose du jeu complet `visite.*` y compris `delete` (lignes 349-350). Tout médecin (profession) reçoit ce rôle. |
...
- **Rôles** (D-003) : le système compte **3 rôles d'habilitation** (`ADMIN_SYSTEME`, `MEDECIN_CHEF`,
  `INFIRMIER`) ; « MEDECIN » est une profession mappée au rôle `MEDECIN_CHEF`, pas un rôle au catalogue.
