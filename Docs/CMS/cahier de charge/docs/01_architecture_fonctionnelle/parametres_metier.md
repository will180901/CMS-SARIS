...
> [[MODULE_16_synchronisation]], [[modele_operationnel]].
...
| **PM-02** | Durée du jeton de rafraîchissement (refresh JWT) | **7** | jours | `security.service.ts` (`REFRESH_TOKEN_TTL = 7*24*60*60`) |
| **PM-03** | Durée du jeton temporaire (étape TOTP) | **5** | minutes | `security.service.ts` (`TEMP_TOKEN_TTL = 5*60`) |
...
> **PM-01 est configurable** : c'est le défaut `auth.session_timeout_minutes` (**480**,
> bornes 5 … 10080) du catalogue `ParametresService` ; un administrateur peut le modifier
> via l'écran Paramètres. Lié à la décision « session unique par utilisateur ».
...
## 9. Volumétrie & cardinalités du système (as-built)
...
|----|---------|-----------------------|-----------------|
| **PM-46** | Nombre de **rôles** | **4** (`ADMIN_SYSTEME`, `MEDECIN_CHEF`, `MEDECIN`, `INFIRMIER`) | `packages/types/src/permissions.ts` (`ROLE_CATALOG`) — voir [[MODULE_02_acces_habilitations]] |
...
- Messagerie (PM-14 à PM-19) → [[MODULE_13_messagerie]].
- Synchronisation (PM-20 à PM-36) → [[MODULE_16_synchronisation]], [[modele_operationnel]].
