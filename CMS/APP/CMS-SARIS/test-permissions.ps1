#
# Suite de tests - Systeme roles & permissions CMS SARIS (modele a 3 ROLES)
# Roles : ADMIN_SYSTEME, MEDECIN_CHEF (absorbe l'admin medical), INFIRMIER.
# Comptes de test (seed, login = nom de famille) :
#   admin/Admin123!  ·  moukanda/Saris2026! (MEDECIN_CHEF)
#   batchi/Saris2026! (INFIRMIER)  ·  ndinga/Saris2026! (INFIRMIER)
# PowerShell (Windows). Prerequis : backend sur http://localhost:3000 + DB seedee.
#
# Realigne le 2026-06-29 sur les 3 roles : les anciens blocs ADMIN_MEDICAL /
# INFIRMIER_DELEGUE / AGENT_RH ont ete retires (roles supprimes). MEDECIN_CHEF a
# desormais audit.read + sous_traitant.* + referentiels CRUD complet.
# A REVALIDER au 1er run live : des endpoints hors-perimetre (accidents-travail,
# suivis-chroniques) ont ete retires du systeme ; ajuster une assertion si besoin.
#
...
# ─────────────────────────────────────────────────────────────────
Write-Host "`nTEST 2 - MEDECIN_CHEF (moukanda / Saris2026!)" -ForegroundColor Cyan
...
Test-Status "GET /personnel"                200 (Call-Api GET /personnel                $medChefToken)
# MEDECIN_CHEF absorbe l'admin medical : il A desormais audit.read (200, plus 403)
Test-Status "GET /admin/audit/actions (MEDECIN_CHEF a audit.read)" 200 (Call-Api GET /admin/audit/actions $medChefToken)
# BLOQUE (pas de droits systeme)
...
# ─────────────────────────────────────────────────────────────────
Write-Host "`nTEST 3 - INFIRMIER (batchi / Saris2026!)" -ForegroundColor Cyan
...
# ─────────────────────────────────────────────────────────────────
Write-Host "`nTEST 4 - Flux dynamique : modif rôle + auth/refresh" -ForegroundColor Cyan
...
# ─────────────────────────────────────────────────────────────────
Write-Host "`nTEST 5 - Garde-fous" -ForegroundColor Cyan
...
# ─────────────────────────────────────────────────────────────────
Write-Host "`nTEST 6 - Routes /auth" -ForegroundColor Cyan
...
# ─────────────────────────────────────────────────────────────────
Write-Host "`nTEST 7 - Separation update/delete sur reentites a statut" -ForegroundColor Cyan
...
if (-not ($adminPerms -contains "role.create") -or -not ($adminPerms -contains "role.update")) {
  Write-Host "  SKIP TEST 7 : admin n'a pas role.create/role.update (perdu via UI ?)" -ForegroundColor Yellow
...
if (-not $testLogin) {
  Write-Host "  Login test_update_only echoue, skip TEST 7" -ForegroundColor Yellow
} else {
  ...
  # (sous_traitant.*) — leur separation est couverte par le TEST 11.
  ...
  # CONTRE-EPREUVE : MEDECIN_CHEF (qui a .delete sur les referentiels) peut bien toggler
  $medChefDelToken = (Login -User "moukanda" -Pass "Saris2026!").accessToken
  $revertStatut = $someSite.statut
  Test-Status "PATCH /sites/:id/statut OK (medecin-chef avec .delete)" 200 (Call-Api PATCH "/referentiels/sites/$($someSite.id)/statut" $medChefDelToken @{statut=$revertStatut})
}
...
Invoke-RestMethod -Uri "$BASE/admin/utilisateurs/$testUserId/statut" -Method PATCH -Headers @{Authorization="Bearer $adminToken"} -ContentType "application/json" -Body (@{statut="DESACTIVE";motif="cleanup test 7"} | ConvertTo-Json -Compress) | Out-Null

} # fin du else "admin a role.create"

# ─────────────────────────────────────────────────────────────────
Write-Host "`nTEST 8 - Flux complet Utilisateurs (CUD + reset + statut + audit)" -ForegroundColor Cyan
...
# ADMIN_MEDICAL a ete fusionne dans MEDECIN_CHEF : on utilise ce role pour le changement.
$medChefRoleId = ($rolesAll | Where-Object { $_.code -eq "MEDECIN_CHEF" } | Select-Object -First 1).id
...
# 8.2 Creation avec login deja pris → 409
...
# 8.3 Creation avec mot de passe trop faible → 400
...
# 8.4 Creation avec mot de passe sans majuscule → 400
...
# 8.5 Creation sans aucun role → 400
...
# 8.6 Set roles (changement) : OK
Test-Status "PATCH /admin/utilisateurs/:id/roles (change roles)" 200 (Call-Api PATCH "/admin/utilisateurs/$newUserId/roles" $adminToken @{roleIds=@($medChefRoleId)})

# 8.7 Reset password : OK
...
# 8.8 Reset password avec mdp trop faible → 400
...
# 8.11 Acces utilisateur (non admin) aux endpoints admin → 403
...
# 8.12 Audit : verifier qu'on retrouve les actions emises ci-dessus
...
# 8.13 Cleanup : desactiver l'user de test pour ne pas polluer
Invoke-RestMethod -Uri "$BASE/admin/utilisateurs/$newUserId/statut" -Method PATCH -Headers @{Authorization="Bearer $adminToken"} -ContentType "application/json" -Body (@{statut="DESACTIVE";motif="cleanup test 8"} | ConvertTo-Json -Compress) | Out-Null

# ─────────────────────────────────────────────────────────────────
Write-Host "`nTEST 9 - Derogations de permissions par utilisateur (GRANT / REVOKE)" -ForegroundColor Cyan
...
# 9.1 Lecture de la ventilation par l'admin
Test-Status "GET /utilisateurs/:id/permissions (admin)" 200 (Call-Api GET "/admin/utilisateurs/$bId/permissions" $adminToken)

# 9.2 Etat initial : batchi n'a PAS audit.read, mais A visite.read (via INFIRMIER)
...
# 9.3 PUT derogations : GRANT audit.read + REVOKE visite.read
...
# 9.4 Ancien JWT batchi : toujours fige (token non rafraichi)
Test-Status "Apres derog, ANCIEN JWT audit (toujours 403)" 403 (Call-Api GET /admin/audit/actions $bTokenV1)

# 9.5 Refresh -> nouveau JWT reflete les derogations
...
# 9.6 La ventilation reflete bien les derogations
...
if (($bd.grants.code -contains "audit.read") -and ($bd.revokes.code -contains "visite.read") -and (-not ($bd.effective -contains "visite.read")) -and ($bd.effective -contains "audit.read")) {
  ...
  $script:FAIL++
  $script:FAILS += "TEST 9.6 ventilation incoherente"
}

# 9.7 Validations : conflit grant+revoke sur le meme code -> 400
...
# 9.9 Permission requise : batchi (sans manage_permissions) ne peut pas gerer les derogations
...
# 9.10 Garde-fou : l'admin ne peut pas se retirer une permission vitale (auto-castration / dernier admin)
...
# 9.11 Assignation GROUPEE (bulk) : accorder audit.read a batchi + ndinga (2e infirmier)
$u2Login  = Login -User "ndinga" -Pass "Saris2026!"
$u2Id     = $u2Login.user.id
$u2RT     = $u2Login.refreshToken
Test-Status "POST bulk GRANT audit.read (2 users)" 200 (Call-Api POST "/admin/utilisateurs/permissions/bulk" $adminToken @{ utilisateurIds=@($bId, $u2Id); code="audit.read"; mode="GRANT"; motif="bulk E2E" })
# ndinga apres refresh peut consulter l'audit
$u2Refresh = Invoke-RestMethod -Uri "$BASE/auth/refresh" -Method POST -ContentType "application/json" -Body (@{refreshToken=$u2RT} | ConvertTo-Json -Compress)
Test-Status "ndinga apres bulk : audit ACCORDE" 200 (Call-Api GET /admin/audit/actions $u2Refresh.accessToken)

# 9.12 Bulk RESET : nettoyage des derogations (retour au comportement des roles)
Test-Status "POST bulk RESET audit.read (2 users)" 200 (Call-Api POST "/admin/utilisateurs/permissions/bulk" $adminToken @{ utilisateurIds=@($bId, $u2Id); code="audit.read"; mode="RESET" })
...
# 9.13 Apres RESET + refresh : batchi retrouve exactement ses droits de role
...
# 9.14 La ventilation ne contient plus aucune derogation
...
if (($bdFinal.grants.Count -eq 0) -and ($bdFinal.revokes.Count -eq 0)) {
  ...
  $script:FAIL++
  $script:FAILS += "TEST 9.14 derogations residuelles"
}

# ─────────────────────────────────────────────────────────────────
Write-Host "`nTEST 10 - Garde-fou dernier administrateur systeme" -ForegroundColor Cyan
...
# Role non-admin a assigner pour les tests (ADMIN_MEDICAL fusionne -> MEDECIN_CHEF).
$nonAdminRoleId = ($rolesA | Where-Object { $_.code -eq "MEDECIN_CHEF" } | Select-Object -First 1).id
...
foreach ($a in $autresAdmins) {
  Invoke-RestMethod -Uri "$BASE/admin/utilisateurs/$($a.id)/statut" -Method PATCH -Headers @{Authorization="Bearer $adminToken"} -ContentType "application/json" -Body (@{statut="DESACTIVE";motif="pre-cleanup test 10 (isoler le dernier admin)"} | ConvertTo-Json -Compress) | Out-Null
}
...
# 10.1 NEG setRoles : admin (seul ADMIN_SYSTEME actif) tente de se retirer le role -> 409
...
if ($adminAfter.roles.code -contains "ADMIN_SYSTEME") {
  ...
  Write-Host "  KO  admin a PERDU ADMIN_SYSTEME ! (restauration automatique)" -ForegroundColor Red; $script:FAIL++; $script:FAILS += "TEST 10.1 admin a perdu ADMIN_SYSTEME"
...
# 10.2 POS setRoles : retirer ADMIN_SYSTEME a test_admin2 quand 2 admins existent -> 200
Test-Status "setRoles : retirer ADMIN_SYSTEME quand 2 admins OK" 200 (Call-Api PATCH "/admin/utilisateurs/$u2Id/roles" $adminToken @{roleIds=@($nonAdminRoleId)})
...
# 10.3 NEG setStatut : un operateur tente de desactiver le DERNIER admin -> 409
...
Invoke-RestMethod -Uri "$BASE/admin/utilisateurs/$u2Id/statut" -Method PATCH -Headers @{Authorization="Bearer $adminToken"} -ContentType "application/json" -Body (@{statut="DESACTIVE";motif="cleanup test 10"} | ConvertTo-Json -Compress) | Out-Null
Invoke-RestMethod -Uri "$BASE/admin/utilisateurs/$opUserId/statut" -Method PATCH -Headers @{Authorization="Bearer $adminToken"} -ContentType "application/json" -Body (@{statut="DESACTIVE";motif="cleanup test 10"} | ConvertTo-Json -Compress) | Out-Null

# ─────────────────────────────────────────────────────────────────
Write-Host "`nTEST 11 - Granularite onglets : sous-traitants & rattachements patient" -ForegroundColor Cyan
...
# Contre-epreuve : MEDECIN_CHEF a sous_traitant.* (absorbe de l'ex admin medical)
$mcTok14 = (Login -User "moukanda" -Pass "Saris2026!").accessToken
Test-Status "GET /sous-traitants OK (medecin-chef)" 200 (Call-Api GET /sous-traitants $mcTok14)
Test-Status "POST /sous-traitants AUTORISE medecin-chef (400 valid, pas 403)" 400 (Call-Api POST /sous-traitants $mcTok14 @{})
...
# Cleanup
Invoke-RestMethod -Uri "$BASE/admin/utilisateurs/$sepUserId/statut" -Method PATCH -Headers @{Authorization="Bearer $adminToken"} -ContentType "application/json" -Body (@{statut="DESACTIVE";motif="cleanup test 11"} | ConvertTo-Json -Compress) | Out-Null
