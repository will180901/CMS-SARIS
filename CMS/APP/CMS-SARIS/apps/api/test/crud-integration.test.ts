...
async function main(): Promise<void> {
  ...
  if (!token) {
    console.log('  (TOTP actif sur admin ? impossible de continuer)')
    return
  }
  const H = {
  ...
  if (!id) {
    console.log('  (création échouée, arrêt)')
    return
  }

  // 3. READ (liste filtrée)
  const list = await fetch(BASE + '/referentiels/pathologies?search=' + code, {
    headers: H,
  })
  const arr = await readJson(list)
  ok(
    'READ : la liste contient la pathologie créée',
    Array.isArray(arr) && arr.some((p: any) => p.id === id),
  )
  ...
  const upd = await fetch(BASE + '/referentiels/pathologies/' + id, {
    method: 'PATCH',
    headers: H,
    body: JSON.stringify({ libelle: 'Test intégration MODIFIÉ' }),
  })
  ok('UPDATE → 2xx', upd.ok)
  const ub = await readJson(upd)
  ok(
    'UPDATE applique le nouveau libellé',
    !!ub && ub.libelle === 'Test intégration MODIFIÉ',
  )

  // 5. TOGGLE statut
  const tog = await fetch(
    BASE + '/referentiels/pathologies/' + id + '/statut',
    {
      method: 'PATCH',
      headers: H,
  ...
  const bad = await fetch(BASE + '/referentiels/pathologies', {
    method: 'POST',
    headers: H,
    body: JSON.stringify({ libelle: 'sans code' }),
  })
  ok('VALIDATION : create sans code → 400', bad.status === 400)

  // 7. DELETE
  const del = await fetch(BASE + '/referentiels/pathologies/' + id, {
    method: 'DELETE',
    headers: H,
  })
  ...
  const list2 = await fetch(BASE + '/referentiels/pathologies?search=' + code, {
    headers: H,
  })
  const arr2 = await readJson(list2)
  ok(
    'DELETE : la pathologie n’apparaît plus',
    Array.isArray(arr2) && !arr2.some((p: any) => p.id === id),
  )

  // ── MODULE CLINIQUE : PATIENTS (CRUD complet avec FK site + catégorie) ──────
  const sites = await readJson(
    await fetch(BASE + '/referentiels/sites', { headers: H }),
  )
  const cats = await readJson(
    await fetch(BASE + '/referentiels/categories-patient', { headers: H }),
  )
  const siteId = Array.isArray(sites) && sites[0] ? sites[0].id : null
  const catId = Array.isArray(cats) && cats[0] ? cats[0].id : null
  ok(
    'PATIENTS : site + catégorie de référence disponibles',
    !!siteId && !!catId,
  )
  if (siteId && catId) {
    ...
    if (pid) {
      const pget = await readJson(
        await fetch(BASE + '/patients/' + pid, { headers: H }),
      )
      ok(
        'PATIENT READ → identité correcte',
        !!pget && JSON.stringify(pget).includes('ZZTEST'),
      )
      const pupd = await fetch(BASE + '/patients/' + pid + '/identite', {
        method: 'PATCH',
        headers: H,
        body: JSON.stringify({ nom: 'ZZTESTMOD' }),
      })
      ok('PATIENT UPDATE identité → 2xx', pupd.ok)
      const pdel = await fetch(BASE + '/patients/' + pid, {
        method: 'DELETE',
        headers: H,
      })
