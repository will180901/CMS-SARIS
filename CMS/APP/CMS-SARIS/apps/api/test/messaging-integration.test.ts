 ...
 *
 * Prérequis : API démarrée + base seedée (admin/Admin123! + comptes personnel liés à
 * 'Saris2026!', même site — cf. packages/db/prisma/seed.ts, section 9.b).
...
async function main(): Promise<void> {
  ...
  // 2e compte de test (seed, personnel du même site MOUTELA que admin — cf. seed.ts 9.b).
  let B = await login('batchi', 'Saris2026!')
  if (!B.token) B = await login('ndinga', 'Saris2026!')
  ok('LOGIN utilisateur B (compte de test)', !!B.token)
  if (!A.token || !B.token) {
    console.log('  (login manquant — arrêt)')
    return
  }
  ...
  ok(
    'A et B sur le même site (requis pour la messagerie)',
    A.user?.siteId === B.user?.siteId,
  )
  ...
  const convR = await fetch(BASE + '/messagerie/conversations', {
    method: 'POST',
    headers: HBJson,
    body: JSON.stringify({ destinataireId: adminId }),
  })
  ...
  if (!convId) {
    console.log('  (conversation échouée — arrêt)')
    return
  }
  ...
  fd.append(
    'contenu',
    'Message de test integration — notifications + chiffrement.',
  )
  const sendR = await fetch(
    BASE + '/messagerie/conversations/' + convId + '/messages',
    {
      method: 'POST',
  ...
  const convs = await (
    await fetch(BASE + '/messagerie/conversations', { headers: HA })
  )
    .json()
    .catch(() => [])
  const convA = Array.isArray(convs)
    ? convs.find((c: any) => c.id === convId)
    : null
  ok('A voit la conversation dans sa liste', !!convA)
  ok(
    'NOTIFICATION : A reçoit le message d’un AUTRE utilisateur (dernier message)',
    !!convA &&
      !!convA.dernierMessage &&
      convA.dernierMessage.deMoi === false &&
      String(convA.dernierMessage.apercu || '').includes(
        'Message de test integration',
      ),
  )

  // A lit le message DÉCHIFFRÉ
  const page = await (
    await fetch(BASE + '/messagerie/conversations/' + convId + '/messages', {
      headers: HA,
    })
  )
    .json()
  ...
  ok(
    'A lit le message déchiffré',
    list.some((m) =>
      String(m.contenu || '').includes('Message de test integration'),
    ),
  )

  // TYPING (« en train d'écrire ») : B signale qu'il écrit → 204
  const typR = await fetch(
    BASE + '/messagerie/conversations/' + convId + '/typing',
    {
      method: 'POST',
      headers: { Authorization: 'Bearer ' + B.token },
    },
...
main()
  .then(() => {
    ...
    // exitCode (pas exit()) : laisse Node fermer proprement les connexions fetch
    // encore ouvertes — un exit() forcé course le nettoyage libuv des handles async
    // (assertion UV_HANDLE_CLOSING observée sous Windows/Node 24 sinon).
    process.exitCode = failed ? 1 : 0
  })
  .catch((e) => {
    console.error('ERREUR test:', e && e.message)
    process.exit(1)
  })
