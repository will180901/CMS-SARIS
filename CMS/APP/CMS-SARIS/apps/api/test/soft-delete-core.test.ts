...
test("modèle dans l'allow-list", () =>
  assert.equal(isSoftDeletable('Patient', allow), true))
test('modèle hors allow-list', () =>
  assert.equal(isSoftDeletable('JournalAudit', allow), false))
test('modèle undefined', () =>
  assert.equal(isSoftDeletable(undefined, allow), false))

console.log('toSoftDeleteUpdate')
test('delete -> update {deletedAt}', () => {
  assert.deepEqual(toSoftDeleteUpdate({ where: { id: 'x' } }, NOW), {
    where: { id: 'x' },
    data: { deletedAt: NOW },
  })
})
...
test('ajoute deletedAt: null par défaut', () => {
  assert.deepEqual(addNotDeletedFilter({ where: { siteId: 's' } }), {
    where: { siteId: 's', deletedAt: null },
  })
})
test('args undefined -> where deletedAt null', () => {
  assert.deepEqual(addNotDeletedFilter(undefined), {
