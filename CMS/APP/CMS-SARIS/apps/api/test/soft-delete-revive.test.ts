...
async function main() {
  ...
  const pre = await readJson(
    await fetch(BASE + '/referentiels/pathologies?search=' + code, {
      headers: H,
    }),
  )
  if (Array.isArray(pre)) {
    for (const p of pre.filter((x: any) => x.code === code)) {
      await fetch(BASE + '/referentiels/pathologies/' + p.id, {
        method: 'DELETE',
        headers: H,
      })
    }
  ...
  // 2) DELETE (soft)
  if (id1) {
    const d1 = await fetch(BASE + '/referentiels/pathologies/' + id1, {
      method: 'DELETE',
      headers: H,
    })
    ...
    ok(
      'après DELETE : absente de la liste',
      Array.isArray(list) && !list.some((p: any) => p.id === id1),
    )
  }
  ...
  ok(
    'RECREATE même code → 2xx (résurrection, pas 500/409)',
    c2.ok,
    'HTTP ' + c2.status,
  )
  ...
  const found = Array.isArray(list2)
    ? list2.find((p: any) => p.code === code)
    : null
  ok('après RECREATE : présente et active', !!found)
  ok(
    'RECREATE applique les nouvelles données (libellé/chronique)',
    !!found && found.libelle === 'Revive test 2',
  )

  // 5) nettoyage
  const cleanupId = b2?.id ?? found?.id
  if (cleanupId)
    await fetch(BASE + '/referentiels/pathologies/' + cleanupId, {
      method: 'DELETE',
      headers: H,
    })
}
