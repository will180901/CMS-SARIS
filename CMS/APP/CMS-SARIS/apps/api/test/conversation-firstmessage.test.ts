...
async function listConvIds(token: string): Promise<string[]> {
  const r = await fetch(BASE + '/messagerie/conversations', {
...
async function main(): Promise<void> {
  // Deux comptes de test du MÊME site (MOUTELA — cf. seed.ts 9.b), qui n'ont pas
  // forcément déjà conversé.
  const U1 = await login('batchi', 'Saris2026!')
  const U2 = await login('ndinga', 'Saris2026!')
  ...
  if (!convId) {
    console.log('  (ouverture échouée — arrêt)')
    return
  }

  if (convBody.created === false) {
    console.log(
      '  (conversation U1↔U2 déjà existante avec messages — test du filtre non concluant, on continue)',
    )
  } else {
    ...
    ok(
      'AVANT 1er message : invisible chez l’ÉMETTEUR (U1)',
      !u1Before.includes(convId),
    )
    ok(
      'AVANT 1er message : invisible chez le DESTINATAIRE (U2)  ← bug corrigé',
      !u2Before.includes(convId),
    )
  }
