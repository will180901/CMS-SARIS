...
} from '../decorators/live-refresh.decorator'
...
export class LiveRefreshInterceptor implements NestInterceptor {
  ...
  intercept(context: ExecutionContext, next: CallHandler): Observable<unknown> {
    const meta = this.reflector.getAllAndOverride<LiveRefreshMeta | undefined>(
      LIVE_REFRESH_KEY,
    ...
    const req = context
      .switchToHttp()
      .getRequest<{ method?: string; user?: { siteId?: string } }>()
    ...
    const opts =
      meta.siteScoped && req.user?.siteId
    ...
    return next
      .handle()
      .pipe(tap(() => this.notif.broadcastLive(meta.type, opts)))
  }
}
