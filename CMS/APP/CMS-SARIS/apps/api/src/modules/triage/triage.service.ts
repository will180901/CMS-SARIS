...
const VISITE_DETAIL_INCLUDE = {
  ...
  site: { select: { id: true, code: true, libelle: true } },
  motifPrincipal: {
    select: { id: true, code: true, libelle: true, triageAllege: true },
  },
...
export class TriageService {
  ...
  private async attachSoignants<T extends { soignantId?: string | null }>(
    visites: T[],
  ) {
    const ids = [
      ...new Set(visites.flatMap((v) => (v.soignantId ? [v.soignantId] : []))),
    ]
  ...
  async findAll(
    ...
    const sorted = [...visites].sort((a, b) => {
      ...
      return (
        new Date(a.dateOuverture).getTime() -
        new Date(b.dateOuverture).getTime()
      )
    })
  ...
  async create(dto: CreateVisiteDto, siteId: string, saisiePar: string) {
    ...
    if (patient.statut !== 'ACTIF')
      throw new ConflictException("Le dossier patient n'est pas actif")
  ...
  async deleteVisite(id: string) {
    ...
    const consultCount = await this.prisma.consultation.count({
      where: { visiteId: id },
    })
    if (consultCount > 0) {
      throw new ConflictException(
  ...
  async updateSoignant(
    id: string,
    dto: UpdateSoignantVisiteDto,
    acteurId: string,
  ) {
    this.assertActor(acteurId)
    const visite = await this.getVisiteOrThrow(id)
    ...
    return this.prisma.$transaction(async (tx) => {
      const updated = await tx.visite.update({
        where: { id },
        data: { soignantId: nouveauId },
      })
      await tx.visiteEvenement.create({
        data: {
          visiteId: id,
  ...
  async createConstantes(
    id: string,
    dto: CreateConstanteVitaleDto,
    saisiePar: string,
  ) {
