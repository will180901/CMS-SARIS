...
export class MeService {
  ...
  async getPreferences(userId: string) {
    const p = await this.prisma.preferenceUtilisateur.findUnique({
      where: { utilisateurId: userId },
    })
    if (p)
      return {
        ...p,
        cguAJour: p.cguVersion === CGU_VERSION,
        cguVersionRequise: CGU_VERSION,
      }
    // Valeurs par défaut (langue = défaut établissement) sans créer de ligne
    const langue =
      (await this.params.getValue('etab.langue_defaut')) || PREF_DEFAULTS.langue
    return {
  ...
  async totpActivate(userId: string, code: string) {
    ...
    if (!cfg)
      throw new BadRequestException(
        "Aucune configuration en attente. Lancez d'abord la configuration.",
      )
    if (cfg.actif)
      throw new BadRequestException(
        'La double authentification est déjà activée.',
    ...
    const { valid } = verifySync({
      token: code,
      secret: decryptSecret(cfg.secretChiffre),
      strategy: 'totp',
      epochTolerance: 30,
    })
    if (!valid)
    ...
    await this.prisma.$transaction(async (tx) => {
      await tx.configurationTotp.update({
        where: { utilisateurId: userId },
        data: { actif: true, activatedAt: new Date() },
      })
