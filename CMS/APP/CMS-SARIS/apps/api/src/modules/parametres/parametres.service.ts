...
export const PARAMETRES_CATALOGUE: ParametreDef[] = [
  ...
  {
    cle: 'auth.tentatives_max',
    type: 'number',
    defaultVal: '5',
    min: 3,
    max: 10,
    description:
      'Tentatives de connexion échouées avant blocage automatique du compte',
    group: 'Sécurité & authentification',
  },
  {
    cle: 'auth.duree_blocage_minutes',
    type: 'duration_minutes',
    defaultVal: '15',
    min: 1,
    max: 1440,
    description:
      'Durée du blocage initial après dépassement du seuil (minutes)',
    group: 'Sécurité & authentification',
  },
  {
    cle: 'auth.session_timeout_minutes',
    type: 'duration_minutes',
    defaultVal: '480',
    min: 5,
    max: 10080,
  ...
  {
    cle: 'notif.app_enabled',
    type: 'boolean',
    defaultVal: 'true',
    description:
      "Activer les notifications dans l'application (cloche, temps réel)",
    group: 'Notifications',
  },
  {
    cle: 'notif.evenements_cliniques',
    type: 'boolean',
    defaultVal: 'true',
    description:
      'Notifier les événements cliniques (visites, consultations, ordonnances, examens…) aux utilisateurs autorisés',
    group: 'Notifications',
  },
  {
    cle: 'notif.sorties_critiques',
    type: 'boolean',
    defaultVal: 'true',
    description:
      'Notifier en priorité les sorties critiques (évacuations, accidents du travail)',
    group: 'Notifications',
  },
  {
    cle: 'notif.evenements_administratifs',
    type: 'boolean',
    defaultVal: 'true',
    description:
      'Notifier les événements administratifs sensibles (comptes, rôles, permissions, paramètres) aux administrateurs',
    group: 'Notifications',
  },
  {
    cle: 'notif.retention_jours',
    type: 'number',
    defaultVal: '30',
    min: 7,
    max: 365,
...
export class ParametresService {
  ...
  async getValue(cle: string): Promise<string> {
    ...
    const ligne = await this.prisma.parametreSysteme.findUnique({
      where: { cle },
    })
  ...
  async getNumber(cle: string): Promise<number> {
    ...
    return Number.isFinite(n)
      ? n
      : Number(CATALOGUE_MAP.get(cle)?.defaultVal ?? 0)
  }
  ...
  async update(cle: string, valeur: string, acteurId: string | null) {
    ...
    const previous = await this.prisma.parametreSysteme.findUnique({
      where: { cle },
    })
    ...
    try {
      await this.prisma.journalAudit.create({
        data: {
          ...
          action: 'UPDATE',
          module: 'parametre',
          entiteType: 'ParametreSysteme',
          entiteId: updated.id,
          avantJson: { cle, valeur: previous?.valeur ?? def.defaultVal },
          apresJson: { cle, valeur },
          statut: 'SUCCES',
        },
      })
    } catch {
      /* l'audit ne bloque jamais */
    }
    ...
    try {
      await this.prisma.journalAudit.create({
        data: {
          ...
          module: 'parametre',
          entiteType: 'ParametreSysteme',
          apresJson: { cle, valeur: def.defaultVal },
          statut: 'SUCCES',
        },
      })
    } catch {
  ...
  private validate(def: ParametreDef, valeur: string) {
    if (def.type === 'number' || def.type === 'duration_minutes') {
      ...
      if (def.min != null && n < def.min)
        throw new BadRequestException(`Valeur minimale : ${def.min}`)
      if (def.max != null && n > def.max)
        throw new BadRequestException(`Valeur maximale : ${def.max}`)
      ...
      if (!ok)
        throw new BadRequestException('Valeur non autorisée pour ce paramètre')
    } else {
      if (valeur.length > 200)
        throw new BadRequestException('Texte trop long (max 200)')
    }
  }
  ...
  async assertPasswordValid(mdp: string) {
    ...
    const fails: string[] = []
    if (mdp.length < p.longueurMin)
      fails.push(`au moins ${p.longueurMin} caractères`)
    ...
    if (p.exigerSpecial && !/[^A-Za-z0-9]/.test(mdp))
      fails.push('un caractère spécial')
    if (fails.length) {
      throw new BadRequestException(
        `Le mot de passe doit contenir : ${fails.join(', ')}.`,
      )
    }
  }
}
