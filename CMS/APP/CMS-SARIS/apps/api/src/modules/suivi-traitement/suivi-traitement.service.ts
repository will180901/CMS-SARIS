...
export class SuiviTraitementService {
  ...
  async findById(id: string) { return this.getOrThrow(id) }

  async create(dto: CreateSuiviTraitementDto, acteurId?: string) {
    const c = await this.prisma.consultation.findFirst({ where: { id: dto.consultationId } })
    ...
    const existing = await this.prisma.raw.suiviTraitement.findUnique({ where: { consultationId: dto.consultationId } })
    ...
    if (existing) {
      await this.prisma.$transaction(async tx => {
        await tx.ficheSuiviTraitement.deleteMany({ where: { suiviTraitementId: existing.id } })
  ...
  async cloturer(id: string, dto: CloturerSuiviTraitementDto) {
    ...
    await this.prisma.suiviTraitement.update({
      where: { id },
      data:  { statut: 'CLOTURE', motifCloture: dto.motifCloture?.trim() || null, closedAt: new Date() },
    })
    return this.getOrThrow(id)
  }
  ...
  async delete(id: string) {
    ...
    await this.prisma.$transaction([
      this.prisma.ficheSuiviTraitement.deleteMany({ where: { suiviTraitementId: id } }),
