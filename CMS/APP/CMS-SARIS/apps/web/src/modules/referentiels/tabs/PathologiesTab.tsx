...
import { usePathologies, useCreatePathologie, useUpdatePathologie, useTogglePathologieStatut, QUERY_KEYS, useDeletePathologie } from '../hooks/useReferentiels'
...
export function PathologiesTab({ canCreate, canUpdate, canDelete }: { canCreate: boolean; canUpdate: boolean; canDelete: boolean }) {
  ...
  const sel = useSelectionLot<PathologieReference>({
    idDe: x => x.id,
    supprimer: id => referentielsApi.pathologies.remove(id),
    invalider: [QUERY_KEYS.pathologies],
  })
...
function PathoRow({ patho, striped, onEdit, onToggle, onDelete, canUpdate, canDelete, sel }: {
  ...
  sel: SelectionLot<PathologieReference>
}) {
  const { t } = useTranslation()
  ...
  return (
    <tr onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}
      {...proprietesLigne(sel, patho, dataRowStyle(striped, hovered || sel.estSelectionne(patho)))}>
      <td style={{ padding: 'var(--espace-2) var(--espace-4)', width: '160px' }}>
        <CaseSelectionLigne sel={sel} ligne={patho}>
        ...
        {showMenu && (
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" style={{ minWidth: '160px', fontSize: '13px' }}>
              {canDelete && <ActionSelectionner onClick={() => sel.entrer(patho)} />}
