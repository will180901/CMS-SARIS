 ...
 * périodiquement l'endpoint public `/health/etat`.
 *
 * ⚠️ NE PAS renommer en `ping` : les bloqueurs de publicité annulent les URL contenant ce
 * mot, l'application se croit alors hors ligne et met les actions en file d'attente.
 *
 ...
 * normalement → badge « Hors ligne » à tort. `/health/etat` est un chemin dédié,
...
export function useServerHealth() {
  ...
  useEffect(() => {
    ...
    async function check() {
      ...
      try {
        const res = await fetch(`${BASE_URL}/health/etat`, { signal: ctrl.signal, cache: 'no-store' })
        if (cancelled) return
